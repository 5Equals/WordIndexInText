...
class ST<Key extends Comparable<Key>, Value> {
    ...
    /**
     * Get the indices associated to the key.
     * @param key the key related to the indices.
     * @return the indices.
     * @throws IllegalArgumentException if the key is invalid.
     */
    public String getIndices(Key key) {
        if (key == null) {
            throw new IllegalArgumentException("The key is invalid.");
        }
        if (isEmpty()) {
     ...
     * @throws IllegalArgumentException if the key is invalid.
     */
    public void put(Key key, Value value, int index)  {
        if (key == null) {
            throw new IllegalArgumentException("The key is invalid.");
        }
        if (value == null) {
        ...
        if (i < this.size && this.keys[i].compareTo(key) == 0) {
            this.values[i] = value;
            String temp = this.indices.get(i);
            temp += "-" + index;
            this.indices.set(i,  temp);
            return;
        }
        ...
        for (int j = this.size; j > i; j--)  {
            ...
            String temp = this.indices.get(j-1);
            this.indices.set(j,temp);
        }
        this.keys[i] = key;
        this.values[i] = value;
        this.indices.set(i, Integer.toString(index));
        this.size++;
